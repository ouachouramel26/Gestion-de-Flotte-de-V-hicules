...
public class KeycloakService {
    ...
    public String createKeycloakUser(String firstName, String lastName, String email, String defaultPassword) {
        ...
        if (response.getStatus() == 201) {
            ...
            log.info("Utilisateur Keycloak créé avec succès (Technicien) : {} (ID: {})", email, userId);
            
            // Assigner le rôle "technicien"
            assignRole(usersResource, userId, "technicien");
            ...
            if (!existingUsers.isEmpty()) {
                // On s'assure qu'il a le rôle technicien même s'il existait déjà
                assignRole(usersResource, existingUsers.get(0).getId(), "technicien");
